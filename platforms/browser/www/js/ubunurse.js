...
function openBD(){
	...
	if(typeof db== "undefined"){
		...
		db=window.openDatabase("Burgos","","Datos de la app",200000); 
		db.transaction(function(tx){
			...
			tx.executeSql("SELECT * FROM Usuarios;",[],function(tx,rs){
				 // alert("usuarios en la tabla: "+rs.rows.length);
					for(var i=0;i<rs.rows.length;i++){  
						var user=rs.rows.item(i);
						//alert("usuario que ya estan en la tabla: "+user.usuario);
...
function continuarDB(){
	  //window.alert("Creada BD Usuarios, necesario no crear las otras?: " + userloggeado);
	   if(userloggeado){
				  //alert("usuario econtrado");
		   ...
		   /*
			...
			tx.executeSql("INSERT INTO Usuarios(usuario,password,loggeado) VALUES('usuario','clave','false');",[],function(tx,rs){
...
function creaTablaUsuarios() {
 ...
 /*function insertLog(){
  ...
  db.transaction(function(tx){  
       tx.executeSql("SELECT * FROM Usuarios;",[],function(tx,rs){
		    // window.alert("Usuario: "+ rs.rows.item(0).loggeado);
		   });  
...
$(document).on('pagebeforeshow', '#page_inicio', pruebas);
function pruebas(){
	//alert("loggeado inicio: "+userloggeado);
}
...
function login(){
	...
	if(!usuario && !clave){
		...
		//usuario=true;
		//alert("userloggeado con boton login: " + $("#nombredeusuario").val()+"/"+$("#clave").val());
		db.transaction(function(tx){  
       tx.executeSql("SELECT * FROM Usuarios WHERE usuario=?;",[$("#nombredeusuario").val()],function(tx,rs){
		   ...
		   if(usuario_log.password==$("#clave").val()){
			   ...
			   validacionLoginError();
			   userloggeado=true;
			  // alert("userloggeado con boton login: " + userloggeado);
			   loggInUsuario();
...
function pruebaSelect(){
	//alert("fallo en select de usuario");
}
function loggInUsuario(){
	 //alert("funcion update login: " + userloggeado);
	db.transaction(function(tx){  
...
function validacionLoginError(){
	...
	if(clave){
		//alert("clave vacio true: "+clave);
		$("#clave").css("border", "2px solid red");
	}else{
		//alert("clave lleno false: "+clave);
		$("#clave").css("border", "0px solid black");
	}
	if(usuario){
		//alert("usuario vacio true: "+clave);
		$("#nombredeusuario").css("border", "2px solid red");
	}else{
		//alert("usuario lleno false: "+clave);
		$("#nombredeusuario").css("border", "0px solid black");
	}
}
function logout(){
	 ...
	 userloggeado=false;
	 //alert("loggeado salida: "+ userloggeado);
...
function updatePaciente(){
	//window.alert("Encontrado registro,update: " +$("#detalle_nombre").val() + ","+ $("#detalle_cip").val() );
	db.transaction(function(tx){  
...
notasTest=$("#notas").val();
 ...
 if(error){
	 //alert("cambiar a error");
	 error=false;
	 //$("#errorBarthel").popup('open');
	//$.mobile.changePage("#error"); 
	$.mobile.changePage( "#errorBarthel", { role: "dialog" } );
 }else{
	 //alert("continuar con calculo Barthel");
	switch(true) {
    case (sumaBarthel<=20):
 ...
 db.transaction(function(tx){  
       ...
       tx.executeSql("INSERT INTO Test(nombreTest,idPaciente,resultadoTest,resultadoColorTest,descripcionResultadoTest,fechaTest,horaTest,notasTest)"+
					" VALUES('Barthel'," +
					 ...
					 "'" + $("#notas").val() + "');");  
     },errorDB,exitoBarthel); 
 }

function exitoBarthel(){
	$.mobile.changePage("resultado.html");
}

//navigator.app.backHistory();
 
 //$.mobile.changePage("#page_resultadoCirculo");  	
}
//Indice de Katz
...
function cargarDatosPfeiffer(){
	...
	db.transaction(function(tx){  
       tx.executeSql("SELECT * FROM Pacientes WHERE cip=?;",[$.id],function(tx,rs){
			...
			//$("#user_nacimiento").append('<strong>Fecha Nacimiento:</strong> ' + paciente.fechaNacimiento);
